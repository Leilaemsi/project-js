...
do {
    ...
    if (TabNbr.includes(randomNumber)==false) {
        TabNbr.push(randomNumber);
        console.log(TabNbr);
    }
} while (TabNbr.length < 3);



var titre= document.querySelectorAll(".Title"),
    prix= document.querySelectorAll(".Price"),
    ImA1 = document.getElementById("Image_A1"),
    ImA2 = document.getElementById("Image_A2"),
    ImA3 = document.getElementById("Image_A3");

for (i = 0; i < 3; i++) {
    titre[i].innerHTML = courses[TabNbr[i]].title;
    prix[i].innerHTML = courses[TabNbr[i]].price;
}
    ImA1.setAttribute("src", courses[TabNbr[0]].image);
    ImA2.setAttribute("src", courses[TabNbr[1]].image);
    ImA3.setAttribute("src", courses[TabNbr[2]].image);